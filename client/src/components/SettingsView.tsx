  ...
  return (
    <button
      ...
      className={cn(
        ...
        checked ? "bg-sidebar-primary" : "bg-foreground/10"
      )}
    >
      <span
        className={cn(
...
export default function SettingsView() {
  ...
  const rows = [
    ...
    { icon: Moon, label: "Dark desk", hint: "Keep the intelligence-desk theme", value: dark, set: (next: boolean) => setTheme(next ? "dark" : "light") },
  ...
  return (
    ...
    >
      <section className="overflow-hidden rounded-xl border border-foreground/[.08] bg-chrome/55 divide-y divide-foreground/[.08]">
        {rows.map((row) => (
          <div key={row.label} className="flex items-center justify-between gap-4 px-4 py-4 sm:px-5">
            <div className="flex min-w-0 items-start gap-3">
              <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-foreground/[.08] bg-foreground/[.03] text-muted-foreground dark:text-neutral-400">
                <row.icon size={15} />
              </span>
              <div className="min-w-0">
                <p className="text-sm font-semibold text-foreground dark:text-white">{row.label}</p>
                <p className="mt-1 text-[12px] text-muted-foreground dark:text-neutral-500">{row.hint}</p>
              </div>
            </div>
      ...
      <section className="mt-6 rounded-xl border border-foreground/[.08] bg-foreground/[.025] p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold text-foreground dark:text-white">Privacy & sources</h2>
