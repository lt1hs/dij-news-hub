...
export default function HeadlinesView({ articles, onOpen }: HeadlinesViewProps) {
  ...
  return (
    <div className="mx-auto max-w-[980px] px-4 pb-32 pt-24 sm:px-6">
      <header className="border-b border-foreground/[.09] pb-8">
        <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-sky-400">Live briefing</p>
        <h1 className="text-3xl font-semibold tracking-[-0.03em] text-foreground dark:text-white">Headlines</h1>
        <h2 className="mt-8 text-sm font-semibold text-foreground dark:text-neutral-100">At a glance</h2>
        <p className="mt-2 max-w-3xl text-[15px] leading-7 text-muted-foreground dark:text-neutral-400">
      ...
      <section aria-label="Ranked headlines" className="divide-y divide-foreground/[.08]">
          ...
          return (
            ...
            >
              <div className="pt-0.5 text-center">
                <span className="block text-2xl font-medium tabular-nums text-foreground/90 dark:text-neutral-200">{index + 1}</span>
              ...
              <div className="min-w-0">
                <h3 className="text-[17px] font-medium leading-6 text-foreground dark:text-neutral-100 transition-colors group-hover:text-sky-300 sm:text-xl sm:leading-7">{article.title}</h3>
                <div className="mt-3 flex items-center gap-3 text-xs text-muted-foreground dark:text-neutral-500">
