    ...
    <div className={cn("mx-auto px-4 pb-32 pt-14 sm:px-6 lg:px-8", wide ? "max-w-[1200px]" : "max-w-[980px]", className)}>
      <header className="mb-8 flex flex-col justify-between gap-4 border-b border-foreground/[.08] pb-6 sm:flex-row sm:items-end">
        <div className="min-w-0">
          ...
          <h1 className="mt-1 text-3xl font-semibold tracking-tight text-foreground dark:text-white">{title}</h1>
          {description && <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground dark:text-neutral-500">{description}</p>}
        </div>
